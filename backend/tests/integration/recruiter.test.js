...
describe('Recruiter Internship Management Integration Tests', () => {
  ...
  describe('GET /api/v1/recruiter/analytics', () => {
    it('returns KPI metrics and dynamic chart datasets', async () => {
      jest.spyOn(User, 'findById').mockReturnValue({
        select: jest.fn().mockResolvedValue({
          _id: mockRecruiterId,
          role: USER_ROLES.RECRUITER,
      ...
      jest.spyOn(RecruiterService, 'getDashboardAnalytics').mockResolvedValue({
        metrics: {
          activeInternships: 3,
        ...
        analytics: {
          applicationsByWeek: [{ week: 'W1', label: 'Aug 1 - Aug 7', count: 10 }],
          applicationsByInternship: [{ internshipId: mockInternshipId, title: 'SWE Intern', count: 15 }],
          statusDistribution: [{ status: 'APPLIED', label: 'Applied', count: 12, color: '#3B82F6' }],
        },
        recentApplications: [],
        upcomingInterviews: [],
        recentInternships: [],
      ...
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(res.body.data.metrics.totalApplications).toBe(25);
      expect(res.body.data.analytics.applicationsByWeek).toHaveLength(1);
    });
  });

  describe('GET /api/v1/recruiter/interviews', () => {
    it('returns scheduled interviews for company', async () => {
      jest.spyOn(User, 'findById').mockReturnValue({
        select: jest.fn().mockResolvedValue({
          _id: mockRecruiterId,
          role: USER_ROLES.RECRUITER,
      ...
      jest.spyOn(RecruiterService, 'getRecruiterInterviews').mockResolvedValue({
        data: [{ _id: 'int123', scheduledAt: new Date().toISOString(), type: 'VIDEO' }],
        page: 1,
        limit: 10,
        total: 1,
        totalPages: 1,
