...
export function EditInternshipPage() {
  ...
  const { currentInternship, loading } = useSelector((state) => state.recruiter);
  ...
  useEffect(() => {
    if (id) {
      dispatch(fetchRecruiterInternshipById(id));
    }
  }, [dispatch, id]);
  ...
  useEffect(() => {
    if (currentInternship) {
      setFormData({
        ...
        category: currentInternship.category || 'Software Engineering',
        remote: currentInternship.remote || 'REMOTE',
        type: currentInternship.type || 'FULL_TIME',
        duration: currentInternship.duration || '3 Months',
        stipendAmount: currentInternship.stipend?.amount || 0,
        isUnpaid: Boolean(currentInternship.stipend?.isUnpaid),
        openings: currentInternship.openings || 1,
        ...
        responsibilitiesText: Array.isArray(currentInternship.responsibilities)
          ? currentInternship.responsibilities.join('\n')
          : '',
        requirementsText: Array.isArray(currentInternship.requirements)
          ? currentInternship.requirements.join('\n')
          : '',
        skills: Array.isArray(currentInternship.skills) ? currentInternship.skills : [],
        status: currentInternship.status || 'DRAFT',
      });
    }
  }, [currentInternship]);
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true);
    try {
      const result = await dispatch(updateInternship({ id, data: payload }));
      if (updateInternship.fulfilled.match(result)) {
        notify.success('Internship updated successfully!');
        navigate('/recruiter/internships');
      } else {
        notify.error(result.payload || 'Failed to update internship.');
      }
    } finally {
      setSaving(false);
    }
          ...
          <Card className="border-slate-200 bg-white shadow-sm">
            ...
            <CardContent className="p-6 space-y-4">
              <div className="flex gap-2">
                ...
                />
                <Button variant="secondary" size="md" type="button" onClick={() => handleAddSkill()}>
                  Add Skill
                </Button>
              </div>
